...
async def create_face_recognition(fr_in: FaceRecognitionCreate, db: Session = Depends(get_db_session)):
    service = FaceRecognitionService(db)
    return service.create(fr_in)


@router.get("/{fr_id}", response_model=FaceRecognitionRead)
async def get_face_recognition(fr_id: int, db: Session = Depends(get_db_session)):
    service = FaceRecognitionService(db)
...
async def list_face_recognition(camera_id: Optional[str] = None, skip: int = 0, limit: int = 100, db: Session = Depends(get_db_session)):
    service = FaceRecognitionService(db)
    return service.list(camera_id=camera_id, skip=skip, limit=limit)


@router.put("/{fr_id}", response_model=FaceRecognitionRead)
async def update_face_recognition(fr_id: int, fr_in: FaceRecognitionUpdate, db: Session = Depends(get_db_session)):
    service = FaceRecognitionService(db)
    fr = service.get(fr_id)
    if not fr:
        raise HTTPException(status_code=404, detail="FaceRecognition not found")
...
async def delete_face_recognition(fr_id: int, db: Session = Depends(get_db_session)):
    service = FaceRecognitionService(db)
    fr = service.get(fr_id)
    if not fr:
        raise HTTPException(status_code=404, detail="FaceRecognition not found")
